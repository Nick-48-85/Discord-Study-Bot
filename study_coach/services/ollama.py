...
Features temperature control based on task type and structured response handling.
"""
...
class PromptType(str, Enum):
    """
    Types of prompts for automatic temperature selection.
    """
    FACTUAL = "factual"  # For factual tasks, summaries, direct answers
    CREATIVE = "creative"  # For creative tasks, brainstorming, alternatives
    DEFAULT = "default"  # Default prompt type


class OllamaService:
    """Service for interacting with the Ollama API with temperature control."""
    ...
    def __init__(self, base_url: str = "http://localhost:11434"):
        ...
        # Define temperature settings for different prompt types
        self.temperature_settings = {
            PromptType.FACTUAL: 0.3,
            PromptType.CREATIVE: 0.7,
            PromptType.DEFAULT: 0.5
        }
        
        # Always use top_p of 0.95 as specified in requirements
        self.top_p = 0.95
    ...
    async def list_available_models(self) -> List[str]:
        ...
        try:
            ...
            print(f"Error listing available models: {type(e).__name__}: {str(e)}")
            return []
    ...
    async def generate_structured_completion(
        ...
        """
        Generate a structured JSON completion using Ollama.
        
        Args:
            prompt: The prompt to send to the model
            prompt_type: Type of prompt for automatic temperature selection
            model: Model to use
            temperature: Optional temperature override (if None, use prompt_type default)
            max_tokens: Maximum tokens to generate
            schema: Optional JSON schema to enforce (not directly supported by Ollama but used for prompt)
            
        Returns:
            Parsed JSON response
        """
        # Determine temperature based on prompt type if not explicitly provided
        if temperature is None:
            temperature = self.temperature_settings[prompt_type]
        
        # If schema provided, append it to the prompt
        if schema:
            schema_str = json.dumps(schema, indent=2)
            prompt += f"\n\nYour response must follow this JSON schema exactly:\n{schema_str}\n"
        else:
            # Always include instruction to return JSON
    ...
    async def generate_completion(
        ...
        # Always use the specified top_p value from requirements
        top_p = self.top_p
    ...
    async def summarize_text(
        ...
        This is a factual task, so we use temperature=0.3.
        """
        ...
        try:
            ...
            try:
                ...
                response = await self.generate_completion(
                    ...
                    model=model,
                    temperature=0.3,  # Still a factual task
                    top_p=self.top_p,
                    max_tokens=500
                )
            except Exception as e:
...
{text[:8000]}  # Limit text length to avoid context limits
...
"""
        ...
        # Extract JSON array from the response
        return self._extract_quiz_questions(response, question_type, num_questions)
    ...
    async def generate_flashcards(
        ...
        This is a creative task, so use temperature=0.7.
        """
        # Limit text size to avoid timeouts
        if len(text) > 12000:
            text = text[:12000] + "..."
...
[
  {{
    ...
    "topic": "[SPECIFIC subject from the text]"
        ...
        return self._extract_flashcards(response, num_cards)
    ...
    async def recommend_next_steps(
        ...
        This uses a combination of analysis (factual) and creativity, so using 0.7.
        """
...
"""
        ...
        response = await self.generate_completion(
            prompt=prompt,
            model=model,
            temperature=0.7,  # Creative task with factual elements
            top_p=self.top_p,
            max_tokens=1000
        )
    ...
    def _extract_json(self, text: str) -> Dict[str, Any]:
        """Helper method to extract JSON from a text response"""
        try:
    ...
    def _extract_quiz_questions(self, response: str, question_type: str, num_questions: int) -> List[Dict[str, Any]]:
        """Extract and format quiz questions from the response."""
        try:
            # Try to find JSON array in the response
            start_idx = response.find('[')
            end_idx = response.rfind(']') + 1
    ...
    def _extract_flashcards(self, response: str, num_cards: int) -> List[Dict[str, str]]:
        """Extract and format flashcards from the response."""
        try:
            # Try to find JSON array in the response
            start_idx = response.find('[')
            end_idx = response.rfind(']') + 1
                ...
                try:
                    ...
                    try:
                        ...
                        return [{"front": "Flashcard generation failed", 
                                "back": "Please try again with shorter or different content",
                                "topic": "Error"}]
            ...
            for line in lines:
                if line.strip().startswith("Front:") or line.strip().lower().startswith("question:"):
                    # Start a new card if we were building one
                    if "front" in current_card:
                        flashcards.append(current_card)
                        current_card = {}
                    
                    # Extract the front text
                    front_text = line.split(":", 1)[1].strip() if ":" in line else line.strip()
                    current_card["front"] = front_text
                    
                elif line.strip().startswith("Back:") or line.strip().lower().startswith("answer:"):
                    # Add the back text to current card
                    back_text = line.split(":", 1)[1].strip() if ":" in line else line.strip()
...
# Legacy compatibility - keep the old name as an alias

# Legacy compatibility - defined at module level

# Legacy compatibility - defined at module level
OllamaEnhancedService = OllamaService
